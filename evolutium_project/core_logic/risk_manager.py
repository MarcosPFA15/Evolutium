...
from . import config # <-- CORREÇÃO APLICADA AQUI
import logging
...
def is_trade_allowed(account_balance: float, open_positions: list, ticker_to_trade: str, trade_value: float, side: str) -> bool:
    ...
    if side == 'BUY':
        ...
        if position_exists:
            logging.warning(f"[RISCO] COMPRA NEGADA: Posição em {ticker_to_trade} já existe. Não são permitidas compras duplicadas.")
            return False
